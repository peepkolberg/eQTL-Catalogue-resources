...
#Add Jerber_2021, Nathan_2022, Cytoimmgen, DICE
...
dice_celltypes = readr::read_tsv("~/Downloads/celltype_cell_counts.DICE.tsv")
dice_map = makeDatasetMetadata("DICE", sample_groups = dice_celltypes$celltype, quant_methods = "ge", study_index_start = 42, dataset_index_start = 752)
new_meta = dplyr::bind_rows(jerber_map, nathan_map, cytoimmgen_map, dice_map)
write.table(new_meta, "data_tables/new_dataset_id_map.tsv", sep = "\t", row.names = F, quote = F)
